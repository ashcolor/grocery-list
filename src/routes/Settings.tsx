...
import type { CategoryDef, LocationDef, StorageLocationDef } from "../context/GroceryContext";
import {
  DndContext,
  closestCenter,
  PointerSensor,
...
function SortableStorageLocationItem({
  loc,
  idx,
  isEditing,
  editLocName,
  ...
  const {
    ...
    transition,
    isDragging,
  } = useSortable({ id: `storage-${loc.name}`, disabled: isEditing });
...
export default function Settings() {
  ...
  const { categories, addCategory, removeCategory, updateCategory, reorderCategories, locations, addLocation, removeLocation, updateLocation, reorderLocations, storageLocations, addStorageLocation, removeStorageLocation, updateStorageLocation, reorderStorageLocations, resetToDefaults, clearAll } = useGrocery();
  ...
  const [editingStorageLocIdx, setEditingStorageLocIdx] = useState<number | null>(null);
  const [editStorageLocName, setEditStorageLocName] = useState("");
  const [editStorageLocEmoji, setEditStorageLocEmoji] = useState("");
  const [addingStorageLoc, setAddingStorageLoc] = useState(false);
  const [newStorageLocName, setNewStorageLocName] = useState("");
  const [newStorageLocEmoji, setNewStorageLocEmoji] = useState("");
  ...
  const handleStorageLocDragEnd = (event: DragEndEvent) => {
    const { active, over } = event;
    if (!over || active.id === over.id) return;
    const fromIdx = storageLocations.findIndex((l) => `storage-${l.name}` === active.id);
    const toIdx = storageLocations.findIndex((l) => `storage-${l.name}` === over.id);
    if (fromIdx !== -1 && toIdx !== -1) reorderStorageLocations(fromIdx, toIdx);
  };
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <h2 className="text-lg font-bold mt-6">保管場所</h2>
      <DndContext sensors={sensors} collisionDetection={closestCenter} onDragEnd={handleStorageLocDragEnd}>
        <SortableContext items={storageLocations.map((l) => `storage-${l.name}`)} strategy={verticalListSortingStrategy}>
          <ul className="flex flex-col gap-2">
            {storageLocations.map((loc, idx) => (
              <SortableStorageLocationItem
                key={loc.name}
                loc={loc}
                idx={idx}
                isEditing={editingStorageLocIdx === idx}
                editLocName={editStorageLocName}
                editLocEmoji={editStorageLocEmoji}
                onEditLocName={setEditStorageLocName}
                onEditLocEmoji={setEditStorageLocEmoji}
                onStartEdit={(i) => { setEditingStorageLocIdx(i); setEditStorageLocName(storageLocations[i].name); setEditStorageLocEmoji(storageLocations[i].emoji); }}
                onSaveEdit={(oldName) => {
                  const name = editStorageLocName.trim();
                  const emoji = editStorageLocEmoji.trim();
                  if (name && emoji) { updateStorageLocation(oldName, { name, emoji }); setEditingStorageLocIdx(null); }
                }}
                onCancelEdit={() => setEditingStorageLocIdx(null)}
      ...
      {addingStorageLoc ? (
        <div className="bg-base-100 rounded-lg px-4 py-3 shadow-sm flex items-center gap-2">
          <input
            type="text"
            className="input input-bordered input-sm w-16 text-center"
            value={newStorageLocEmoji}
            onChange={(e) => setNewStorageLocEmoji(e.target.value)}
            placeholder="📍"
            autoFocus
          />
          <input
            ...
            onKeyDown={(e) => {
              if (e.key === "Enter") {
                const name = newStorageLocName.trim();
                const emoji = newStorageLocEmoji.trim();
                if (name && emoji && !storageLocations.some((l) => l.name === name)) {
                  addStorageLocation({ name, emoji }); setNewStorageLocName(""); setNewStorageLocEmoji(""); setAddingStorageLoc(false);
                }
              }
            }}
            placeholder="保管場所名"
          ...
          <button className="btn btn-sm btn-ghost" onClick={() => setAddingStorageLoc(false)}>
            <Icon icon="mdi:close" className="size-4" />
          </button>
        </div>
      ) : (
        <button className="btn btn-outline btn-sm self-start" onClick={() => setAddingStorageLoc(true)}>
          <Icon icon="mdi:plus" className="size-4" />
          保管場所を追加
        </button>
      )}
